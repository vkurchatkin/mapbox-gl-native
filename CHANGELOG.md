...
- The user dot now moves smoothly between user location updates. In user tracking mode, the user dot stays in a fixed position within MGLMapView while the map pans smoothly. A new property, `userLocationVerticalAlignment`, determines the user dot’s fixed position. ([#1582](https://github.com/mapbox/mapbox-gl-native/pull/1582), [#3589](https://github.com/mapbox/mapbox-gl-native/pull/3589))
